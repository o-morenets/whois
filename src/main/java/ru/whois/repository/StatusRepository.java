...
public class StatusRepository {
    ...
    private String findStatusInfo = "select o.created, o.updated, o.registrarcr, o.registrar, os.status, os.reason " +
            "from r_domain d" +
            " left join r_object o on (o.id = d.oid)" +
            "left join r_object_status os on (os.objectid = d.oid)" +
            "where d.name = ?";
...
class StatusRowMapper implements RowMapper<StatusInfo> {
    ...
    public StatusInfo mapRow(ResultSet rs, int rowNum) throws SQLException {
        ...
        return new StatusInfo(rs.getString("created"),
                rs.getString("updated"), rs.getString("registrarcr"), rs.getString("registrar"), rs.getString("status"));
    }
}
